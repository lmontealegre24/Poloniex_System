# -*- coding: utf-8 -*-
"""
Created on Fri Mar  9 15:57:04 2018
...
def coint(endog, det_order=0, k_ar_diff=1, print_test = False, verbose=False):
    ...
    if print_test:
        ...
        coint_test = pd.DataFrame(crit_val, columns = ['10%', '5%', '1%'])

        coint_test['test'] = test_stat

        coint_test.index.name = 'H_i'

        print(coint_test[['test', '10%', '5%', '1%']])
    ...
    if colnames is None:
        ...
        result.v_norm = pd.DataFrame(v_norm, columns = colnames)
